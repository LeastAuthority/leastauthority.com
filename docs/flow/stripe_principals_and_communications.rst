...
============
 ...
 During Stripe_ -processed signup for our services, three Principal Actors send
 Messages to each other.  This document describes the expected behavior of these
 Principals and Messages in each of several cases.
 ...
 A note on the format:
   Each Case contains a sequence of behaviors the reader can engage in,
   in order to (re)produce the case. This reproduction will produce output
   similar to that included in the Case.

Caveat Emptor
-------------

 This is a Message-Centric analysis.  That seemed to be a reasonable framework
 for carving up the system.  I make no claim that it's in-any-sense optimal.
